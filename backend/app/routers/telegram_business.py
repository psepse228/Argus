...
from app.telegram.bot_client import TelegramSendError, send_bot_message, send_message, verify_webhook_signature
...
def telegram_business_webhook(update: dict, request: Request):
    ...
    # Proactive Argus Brain (2026-08-08): a plain personal message to the
    # bot -- completely unrelated to the business-connection relay below.
    # Only shape handled here is "/start <code>", generated by Argus's own
    # "Подключить Telegram" button and pasted back by the user tapping the
    # deep link. Anything else on this path is ignored, not an error --
    # someone poking the bot directly with a random message has nothing to
    # link.
    direct_message = update.get("message")
    if direct_message and isinstance(direct_message.get("text"), str) and direct_message["text"].startswith("/start"):
        parts = direct_message["text"].split(maxsplit=1)
        code = parts[1].strip() if len(parts) > 1 else ""
        if code:
            matched = client.table("tenant_users").select("id").eq("telegram_link_code", code).execute().data
            if matched:
                client.table("tenant_users").update({
                    "telegram_chat_id": direct_message["chat"]["id"], "telegram_link_code": None,
                }).eq("id", matched[0]["id"]).execute()
                try:
                    send_bot_message(direct_message["chat"]["id"], "Готово — Argus Brain теперь может писать вам сюда.")
                except TelegramSendError:
                    pass  # best-effort confirmation -- the link itself already succeeded regardless
        return {"ok": True}
