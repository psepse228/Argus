 ...
 * -- with no chat and no справка button. This is what Клиенты and Лиды's
 * "Открыть карточку клиента" open now; actually working a client (chat,
 * справка, Cortège+) is a deliberate step into Мастерская via the button
 * below, not something every click into a client should launch straight into. */
export function ClientInfoCard({
  clientId, onBack, onOpenWorkspace,
}: {
  clientId: string;
  onBack: () => void;
  ...
  return (
    <div className="section-enter" style={{ flex: 1, minHeight: 0, display: "flex", gap: 16 }}>
      <div style={{ width: 320, flexShrink: 0, display: "flex", flexDirection: "column", gap: 14, overflowY: "auto" }}>
        <button
          onClick={onBack}
        ...
        {!selected ? (
          <div className="glass-panel" style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 10 }}>
